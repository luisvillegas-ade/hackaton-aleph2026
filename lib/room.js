...
// Sube un solo archivo ya ubicado dentro de la carpeta del proyecto,
// conservando su ruta relativa. Lo usa el vigilante cuando detecta cambios.
async function addOne(drive, projectRoot, baseName, rel) {
  const data = await fs.promises.readFile(path.join(projectRoot, rel))
  const drivePath = toDrivePath(path.join(baseName, rel))
  await drive.put(drivePath, data)
  return { name: drivePath.slice(1), size: data.length }
}

async function removeOne(drive, baseName, rel) {
  const drivePath = toDrivePath(path.join(baseName, rel))
  await drive.del(drivePath).catch(() => {})
  return { name: drivePath.slice(1) }
}
